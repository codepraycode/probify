...
type QuestionCardProps = {
    question: Question;
    selectedAnswer: QuestionSelectedOption | undefined;
...
export const QuestionCard = observer(
    ({
        ...
        const selectedOption = selectedAnswer;//sourceAnswer(questionObj.id);
            ...
            <div className="space-y-6 rounded-2xl border border-stroke p-6 shadow-two dark:border-stroke-dark dark:bg-dark">
                ...
                {type === "mcq" && (
                    <div className="grid gap-4 sm:grid-cols-2">
                        {options.map((opt, idx) => {
                            const isSelected =
                                selectedOption?.value === opt.plain;
                            return (
                                <button
                                    key={idx}
                                    type="button"
                                    onClick={() =>
                                        onAnswer(questionObj.id, opt.plain)
                                    }
                                    className={clsx(
                ...
                {type === "fill" && (
                    <input
                        ...
                        onChange={(e) =>
                            onAnswer(questionObj.id, e.target.value)
                        }
